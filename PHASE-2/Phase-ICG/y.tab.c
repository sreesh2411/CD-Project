...
{
       0,    80,    80,    80,    80,    80,    80,    80,    85,    85,
      86,    86,    86,    86,    87,    87,    87,    87,    89,    89,
      90,    90,    90,    90,    91,    91,    91,    91,    91,    91,
      91,    93,    93,    93,    93,    95,    96,    97,    98,   103,
     104,   104,   105,   105,   106,   110,   110,   110,   110,   110,
     115,   116,   121,   122,   122,   123,   123,   124,   127,   127,
     127,   127,   127,   132,   133,   138,   139,   139,   140,   140,
     141,   143,   143,   143,   146,   149,   151,   151,   151,   151,
     154,   162,   162,   171,   174,   177,   185,   190,   190,   205,
     207,   207,   220,   220,   232,   234,   234,   247,   247,   264,
     266,   266,   281,   283,   283,   283,   283,   283,   283,   287,
     287,   287,   287,   287
};
#endif
...
static const char *const yytname[] =
{
  ...
  "$@10", "$@11", "U", "$@12", "S", "$@13", "$@14", "S1", "$@15", "$@16",
  "$@17", "$@18", "$@19", "$@20", "S2", "$@21", "$@22", "$@23", "$@24",
  "$@25", "$@26", "EL", "$@27", "$@28", "Arg", "C", "B", "$@29", "assign",
  "$@30", "E", "$@31", "$@32", "F", "$@33", "$@34", "T", "$@35", "Q",
  "$@36", "$@37", "$@38", "relop", YY_NULLPTR
};
#endif
...
#line 1583 "y.tab.c"
  ...
  case 7:
#line 80 "parser.y"
                                                                                                                     {(yyval.nam).nd = mknode(NULL,(yyvsp[-3].nam).nd,"start");printf("\n\nxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");//printf("\n\t\t\tSyntax Tree in Inorder traversal\n-----------------------------------------------------------------------------------\n");
//printtree($$.nd);
printf("\n\n");
add('t');
optimized();}
...
#line 1848 "y.tab.c"
  ...
  case 62:
#line 130 "parser.y"
                                             {(yyval.nam).nd=mknode((yyvsp[-8].gen).nd,(yyvsp[-3].nam).nd,"WHILE");
...
#line 190 "parser.y"
	  ...
	  printf("= \t %s\t %s \n",(yyvsp[-1].nam).name,(yyvsp[-4].nam).name);
		...
		strcpy(intermediate_code[code].res,(yyvsp[-4].nam).name);
		strcpy(intermediate_code[code].op1,(yyvsp[-1].nam).name);
		code++;
		} 
	  else {printf("Variable not declared at line no: %d\n", yylineno);
	  exit(0);}}
...
#line 207 "parser.y"
	...
	if(i!=0 && j!=0) {printf("%s\t%s\t%s\t%s\n","+",(yyvsp[-3].nam).name,(yyvsp[0].nam).name,(yyval.nam).name);strcpy(intermediate_code[code].op,"+");
		strcpy(intermediate_code[code].res,(yyval.nam).name);
		strcpy(intermediate_code[code].op1,(yyvsp[-3].nam).name);
		strcpy(intermediate_code[code].op2,(yyvsp[0].nam).name);
		code++;}
	 else {printf(" Variable not declared at line no: %d\n", yylineno);exit(0);}}
#line 2067 "y.tab.c"
    break;
...
int main()
{
	...
	x.val=10;
	printf(GRN "\n\n*****************************************************************************************************\n" RESET);
	printf(CYN "\t\t\t\tIntermediate code (Quadruple Format)\n" RESET);
	printf(GRN "*****************************************************************************************************\n" RESET);
	yyparse();
	
	printf(GRN "Successfully parsed!\n" RESET);	
	printf("\nxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\n");
	//printf(GRN "\n\n*****************************************************************************************************\n" RESET);
	/*printf(CYN "\t\t\t\t\tSymbol Table\n");
	printf(GRN "*****************************************************************************************************\n" RESET);	
	printf("\nSYMBOL \t\t\t\t TYPE \t\t\t\t CLASS \t\t\t\t LINE NO.\n");
	printf("------------------------------------------------------------------------------------------------------------\n" RESET);
	*/
	int i=0;
	/*
...
{
	printf(RED "Error in the program. Please try again!\n" RESET);
	exit(0);
}
...
for(i=0;i<code;i++)
    ...
    /*printf(BLU "\n----------------------------------------------------------------------------------------\n" RESET);
    printf(YEL "\t\t\t\tCODE after Optimization\n" RESET);
    printf(BLU "----------------------------------------------------------------------------------------\n\n" RESET);
